...
namespace DemoDatabaseCompare.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("DemoDatabaseCompare") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "DemoDatabaseCompare", DisplayName = "DemoDatabaseCompare API", Resources = { "DemoDatabaseCompare" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["DemoDatabaseCompare_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["DemoDatabaseCompare_Web:RootUrl"]!.EnsureEndsWith('/');

            /* DemoDatabaseCompare_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["DemoDatabaseCompare_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["DemoDatabaseCompare_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["DemoDatabaseCompare_Swagger:RootUrl"]?.TrimEnd('/');
